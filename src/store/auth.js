...
export default { 
    ...
    actions: {
        async getRegisterData({ commit, dispatch}, payload){
            const APIkey = "AIzaSyA0hv_dkaziRnedSOOjTSn3EyY6OQoQj4Y";
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                const {data} = await axios.post(
                    `https://vue-js-project-e0ded-default-rtdb.firebaseio.com/user.json?auth=${ state.token }`,payload
                );
        ...
        async getLoginData({ commit, dispatch }, payload) {
            ...
            try {
                ...
                // console.log("Local ID", data.localId); // Debug
        ...
        async getUser({ commit }, payload) {
            try {
                ...
                (
                    "https://vue-js-project-e0ded-default-rtdb.firebaseio.com/user.json"
                )
